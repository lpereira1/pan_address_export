...
def firewall_import(import_fw, import_dict, import_username, import_password, commit):
    ...
    print(f"Exported {len(import_dict['addresses'])} address objects and {len(import_dict['groups'])} from { import_fw }")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-euser','--export_username', help="Username to login as on Exporting Firewall. If not set will default to import firewall")
    parser.add_argument('-epass','--export_password', help='Password to login as on Exporting Firewall. If not set will default to import firewall')
    parser.add_argument('-output','--output', action='store_true', help='True/False : Do you want the output of the export as a text file (in JSON)')
    parser.add_argument('-file','--output_file',default='exported_json.json', help='if output yes: File name to export to')
    parser.add_argument('-group','--group', help='Name of the Group you want to export')
    parser.add_argument('-commit','--commit', action='store_true', default=False, help='You can choose to if you wish to commit the change from this script')
